        ...
        if self.attributes is not None:
            for attr in self.attributes:
                ...
                if type_mapping.get(attr_type) and hasattr(attr, "dimensions") and attr.dimensions == "*":
                    ...
                    bp = get_blueprint(address).to_dict()
                ...
                if type_mapping.get(attr_type) and hasattr(attr, "dimensions") and attr.dimensions == "*":
                    ...
                    if address == "any":
                        raise BadRequestException(
                            "The SQL repository does not support blueprints using the 'any' type",
                            debug=f"An 'any' type was encountered in blueprint {self.path}",
                        )
                    bp = get_blueprint(address).to_dict()
                    child_blueprint = SQLBlueprint.from_dict(bp)
